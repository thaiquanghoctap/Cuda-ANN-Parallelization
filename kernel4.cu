...
__global__ void fusedMatrixMul(float* __restrict__ A, float* __restrict__ B, float* __restrict__ C, 
                               int nrows_A, int ncols_A, int ncols_B) {
    ...
    // Fusion: Load and compute within the same kernel
    for (int t = 0; t < (ncols_A + BLOCK_SIZE - 1) / BLOCK_SIZE; ++t) {
        // Load A and B into shared memory
        if (row < nrows_A && t * BLOCK_SIZE + threadIdx.x < ncols_A) {
            Asub[threadIdx.y][threadIdx.x] = A[row * ncols_A + t * BLOCK_SIZE + threadIdx.x];
        } else {
            Asub[threadIdx.y][threadIdx.x] = 0.0f;
        }

        if (col < ncols_B && t * BLOCK_SIZE + threadIdx.y < ncols_A) {
            Bsub[threadIdx.y][threadIdx.x] = B[(t * BLOCK_SIZE + threadIdx.y) * ncols_B + col];
        } else {
        ...
        // Compute: Loop unrolling for BLOCK_SIZE = 16
        #pragma unroll 4
    ...
    // Store the result
    if (row < nrows_A && col < ncols_B) {
        C[row * ncols_B + col] = CValue;
    }
}
...
void matmul(float* A, float* B, float* C, int m, int n, int k) {
    ...
    // Kích thước grid và block
    dim3 blockDim(BLOCK_SIZE, BLOCK_SIZE);
    dim3 gridDim((k + BLOCK_SIZE - 1) / BLOCK_SIZE, (m + BLOCK_SIZE - 1) / BLOCK_SIZE);

    // Gọi kernel
    fusedMatrixMul<<<gridDim, blockDim, 2 * BLOCK_SIZE * BLOCK_SIZE * sizeof(float)>>>(d_A, d_B, d_C, m, n, k);
...
void train(float* A0, float* Y, float* W1, float* b1, float* W2, float* b2, float* W3, float* b3,
    ...
    const char* filename = "./log/kernel4-training-log.csv";
    printf("\nSTARTING TRAIN ...\n");
    // MALLOC FOR FORWARD
          ...
          int n_samples, int input_layer_nodes, int hidden_layer1_nodes, int hidden_layer2_nodes, int output_layer_nodes, int n_epochs)
{
    ...
    log_accuracy("Kernel_4", n_epochs, correct_predictions, n_samples, macro_f1);
    // Giải phóng bộ nhớ
    free(Z1); free(A1); free(Z2); free(A2); free(Z3); free(A3);
}
