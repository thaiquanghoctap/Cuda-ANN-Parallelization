...
__global__ void matrixMultiplySharedAtomicFusion(float* A, float* B, float* C, int M, int K, int N) {
    // Khai báo bộ nhớ chia sẻ cho các khối dữ liệu của A và B
    __shared__ float sharedA[TILE_WIDTH][TILE_WIDTH];
    __shared__ float sharedB[TILE_WIDTH][TILE_WIDTH];

    // Tính toán chỉ số hàng và cột cho ma trận C
    int row = blockIdx.y * TILE_WIDTH + threadIdx.y;
    int col = blockIdx.x * TILE_WIDTH + threadIdx.x;

    // Khởi tạo giá trị tích lũy cho C[row, col]
    float value = 0.0f;

    // Duyệt qua các khối của ma trận A và B
    for (int t = 0; t < (K - 1) / TILE_WIDTH + 1; ++t) {
        // Nạp dữ liệu từ A và B vào bộ nhớ chia sẻ
        ...
        // Thực hiện tính toán trên các khối đã nạp
        #pragma unroll
    ...
    // Sử dụng atomicAdd để cộng dồn giá trị vào C
    if (row < M && col < N) {
...
void matmul(float* h_A, float* h_B, float* h_C, int M, int K, int N) {
    ...
    cudaMemset(d_C, 0, M * N * sizeof(float)); // Khởi tạo C về 0
    ...
    // Gọi kernel
    matrixMultiplySharedAtomicFusion<<<dimGrid, dimBlock>>>(d_A, d_B, d_C, M, K, N);
...
void train(float* A0, float* Y, float* W1, float* b1, float* W2, float* b2, float* W3, float* b3,
    ...
    const char* filename = "./log/kernel5-training-log.csv";
    printf("\nSTARTING TRAIN ...\n");
    // MALLOC FOR FORWARD
    ...
    log_details("Kernel 5", n_epochs, matmul_time, add_bias_time, relu_time, softmax_time, transpose_time, matsub_time, sum_rows_time, relu_derivative_time, "./log/time-details.csv");
}
          ...
          int n_samples, int input_layer_nodes, int hidden_layer1_nodes, int hidden_layer2_nodes, int output_layer_nodes, int n_epochs)
{
    ...
    log_accuracy("Kernel_5", n_epochs, correct_predictions, n_samples, macro_f1);
    // Giải phóng bộ nhớ
    free(Z1); free(A1); free(Z2); free(A2); free(Z3); free(A3);
}
